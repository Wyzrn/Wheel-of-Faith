<script lang="ts">
  ...
  // ── Route mode ────────────────────────────────────────────────────────────
  // The same component renders three URLs:
  //  - '/'              → routeMode='menu'   → main menu visible
  //  - '/spin'          → routeMode='spin'   → wheel + spinning UI
  //  - '/character/new' → routeMode='result' → name input + final card
  // We sync the URL when state transitions happen so /spin → /character/new
  // → / mirrors the player's progress. The initial state values use
  // routeMode to seed showMenu / showNameScreen on mount.
  type RouteMode = 'menu' | 'spin' | 'result'
  let { routeMode = 'menu' as RouteMode }: { routeMode?: RouteMode } = $props()

  // ── State declarations ────────────────────────────────────────────────────
  // Initial values are seeded from routeMode. Spin/result modes always
  // hide the menu; only the menu route shows it on mount.
  let showMenu = $state(routeMode === 'menu')
  ...
  // Hard cap on wildcard-granted bonus weapons/armors per run. Powers are not
  // capped per the spec. Reset alongside other run state.
  ...
  function handleSpinComplete(resultIndex: number, resultLabel: string) {
    ...
    if (def.category === 'race') {
      ...
      const zzzMinusIdx = TIER_ORDER.indexOf('ZZZ-' as TierGrade)
      const godlyMinusIdx = TIER_ORDER.indexOf('Godly-' as TierGrade)
      const infMinusIdx = TIER_ORDER.indexOf('Infinite-' as TierGrade)
  ...
  function handleWildcardContinue() {
    ...
    if (otype === 'reroll') {
      ...
      // Cap bonus weapons/armors at WILDCARD_BONUS_ITEM_CAP per run; powers uncapped.
  ...
  function handleStartOver() {
    ...
    if (wildcardRevealTimeout) { clearTimeout(wildcardRevealTimeout); wildcardRevealTimeout = null }
    wildcardPhase = 'idle'
    wildcardBonusGranted = { weapon: 0, armor: 0 }
    corruptionScore = 0
    raceOverride = null
    rivalMode = false
    rivalPhase = 'p1'
          ...
          {#if isRevealed}
            ...
            {#if last}
              ...
              {@const statDesc = STAT_DESCRIPTION_CATEGORIES.has(last.category)
                ...
                : ''}
              {@const extraDesc = EXTRA_DESCRIPTION_CATEGORIES.has(last.category)
