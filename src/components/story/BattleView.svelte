<script lang="ts">
  ...
  let selectedChar     = $state<StoryRosterEntry | null>(null)   // lead fighter
  let playerChar       = $state<BattleCharacter | null>(null)
  let enemyChar        = $state<BattleCharacter | null>(null)
  let currentEnemy     = $state<Enemy | null>(null)

  // Wave tracking
  let allWaves         = $state<Enemy[][]>([])
  let waveIdx          = $state(0)
  let enemyIdx         = $state(0)
  let currentPlayerHp  = $state(0)
  ...
  let previewFirstEnemy = $derived(previewWaves[0]?.[0] ?? null)
  let ec = $derived(currentEnemy ? gradeColor(currentEnemy.grade) : gradeColor(previewFirstEnemy?.grade ?? 'F'))
  ...
  function detectAnim(line: string): { type: string; color: string; direction: AnimDir } | null {
    const direction: AnimDir =
      ...
      (enemyChar  && line.startsWith(enemyChar.name))  ? 'rtl' : 'center'
  ...
  function startNextFight() {
    const enemy = allWaves[waveIdx]?.[enemyIdx]
    if (!enemy) { onAllWavesDone(); return }

    currentEnemy = enemy
    enemyChar = buildEnemyChar(enemy)

    const playerForFight: BattleCharacter = { ...playerChar!, hp: currentPlayerHp }
    p1DisplayHp = currentPlayerHp
    p2DisplayHp = enemyChar.hp

    rounds = simulateBattle(playerForFight, enemyChar, 10)
    roundIdx = 0

    const totalWaves = allWaves.length
    const totalInWave = allWaves[waveIdx].length
    const waveLabel = totalWaves > 1 ? ` · Wave ${waveIdx + 1}/${totalWaves}` : ''
    const enemyLabel = totalInWave > 1 ? ` [${enemyIdx + 1}/${totalInWave}]` : ''
    logLines = [...logLines, `―― ${enemy.name}${enemyLabel}${waveLabel} ――`]
    scrollLog()

    timeoutId = setTimeout(playRound, 400)
  }

  function onFightEnd() {
    const lastRound = rounds.at(-1)!
    const wonFight = lastRound.winner === 'p1'
    currentPlayerHp = Math.max(0, lastRound.p1Hp)
    ...
    // Accumulate drops for this kill
    const d = rollDrops(currentEnemy!)
    accDrops = {
    ...
    if (enemyIdx < waveEnemies.length) {
      timeoutId = setTimeout(startNextFight, 700)
    } else {
      waveIdx++
      if (waveIdx < allWaves.length) {
        const restored = Math.round(playerChar!.maxHp * 0.5)
        const actual = Math.min(restored, playerChar!.maxHp - currentPlayerHp)
        currentPlayerHp = Math.min(playerChar!.maxHp, currentPlayerHp + restored)
        p1DisplayHp = currentPlayerHp
        const restoreStr = actual > 0 ? ` · HP +${formatHp(actual)} restored` : ''
        logLines = [...logLines, `―― ★ Wave ${waveIdx} cleared!${restoreStr} ――`]
        scrollLog()
        enemyIdx = 0
        timeoutId = setTimeout(startNextFight, 1200)
      } else {
        onAllWavesDone()
      }
    }
  ...
  function startFight() {
    if (!teamFighter) return
    selectedChar = teamFighter
    playerChar = buildBattleCharacter(selectedChar.spins, selectedChar.name)
    currentPlayerHp = playerChar.hp
  ...
  // Summary of wave/enemy counts for pick phase preview
  function waveLabel(wave: typeof previewWaves[0]): string {
    const normals = wave.filter(e => e.type === 'normal').length
    const elites  = wave.filter(e => e.type === 'elite').length
    const bosses  = wave.filter(e => e.type === 'boss').length
    const parts = []
    if (normals) parts.push(`${normals} enemy${normals > 1 ? 's' : ''}`)
    if (elites)  parts.push(`${elites} elite${elites > 1 ? 's' : ''}`)
    if (bosses)  parts.push(`${bosses} boss${bosses > 1 ? 'es' : ''}`)
    return parts.join(' + ')
  }
</script>
    ...
    <div class="mb-5">
      ...
      <div class="obsidian-slab rounded-xl px-3 py-2.5" style="border: 1px solid {ec}33;">
        {#each previewWaves as wave, wi}
          <div class="flex items-center gap-2 {wi > 0 ? 'mt-2 pt-2 border-t' : ''}" style="{wi > 0 ? 'border-color: rgba(255,255,255,0.06);' : ''}">
            {#if previewWaves.length > 1}
              <span class="font-mono text-xs flex-shrink-0" style="color: var(--color-outline); min-width: 50px;">Wave {wi + 1}</span>
            {/if}
        ...
        {/each}
        {#if previewWaves.length > 1}
          <p class="font-mono text-xs mt-2" style="color: #34d399; font-size: 10px;">50% HP restored between waves</p>
        {/if}
      </div>
    </div>
    ...
    {:else}
      ...
      <div class="fixed bottom-0 left-0 right-0 px-4 pt-3 z-20"
        ...
        <div class="max-w-md mx-auto">
          <button onclick={startFight} disabled={!selectedTeam || !teamFighter}
      ...
      <div class="w-full rounded-xl overflow-hidden mb-4" style="border: 1px solid rgba(240,192,64,0.12); background: #0d0d16;">
        ...
        <div bind:this={logEl} class="overflow-y-auto px-4 py-3" style="max-height: 280px; scroll-behavior: smooth;">
          ...
          {#each logLines as line}
            {#if line.startsWith('―― ★')}
              <p class="text-xs mt-3 mb-2 text-center font-bold tracking-wide" style="font-family: 'JetBrains Mono', monospace; color: #34d399;">{line}</p>
    ...
    <div class="w-full max-w-md rounded-2xl overflow-hidden"
      ...
      <div class="px-5 py-6 text-center">
        ...
        {:else}
          ...
          <p style="font-family: 'Cinzel', serif; font-size: clamp(1.4rem, 5vw, 2rem); font-weight: 900; color: #ef4444; letter-spacing: 0.12em;">{currentEnemy?.name ?? 'Enemy'} WINS!</p>
          <p class="mt-1 text-sm" style="color: #9a907b;">{selectedChar?.name} was defeated. Try again.</p>
        {/if}
